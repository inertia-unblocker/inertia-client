...
export default function ResendModal({ ...props }: ResendModalProps) {
	...
	const closeAndResend = async () => {
		...
		const response = await request('POST', '/api/user/resend', {}, {
			email: emailValue,
		});
		...
		if (response.err) {
			if (response.message == 'USER_NOT_FOUND') {
				...
				return;
			} else if (response.message == 'USER_ALREADY_VERIFIED') {
				props.closeHandler();
				props.parentCloseHandler();

				setError('Email already verified. You can just log in.');
				setErrorModalVisible(true);
				setLoadingModalVisible(false);
				return;
			}
		...
		<>
			...
			>
				...
				<nextUI.Modal.Footer>
					...
					<nextUI.Button onClick={closeAndResend} auto>
						Resend
					</nextUI.Button>
				</nextUI.Modal.Footer>
			</nextUI.Modal>
